...
function [R, T, inliers, trialcount] = loransac(K, X, x, fittingfn, refinefn, s, t, p, idsampl, maxTrials)
    
    [~, npts] = size(x);
    
    if nargin < 8; p = 0.99; end % Desired probability of choosing at least one sample free from outliers
    if nargin < 9; idsampl = 1:npts; end;
    if nargin < 10; maxTrials = 1000000; end;
    ...
    while N > trialcount
        ...
        if ninliers > bestscore1    % Largest set of inliers so far...
            
            bestscore1 = ninliers;  % Record data for this model
            
            [R, T] = refinefn(R, T, X, x, K, t);
            [inliers, R, T] = evalRT(K, R, T, X, x, t);
            ninliers = length(inliers);
            
            if ninliers > bestscore2
                bestscore2 = ninliers;  % Record data for this model
                bestinliers = inliers;
                bestR = R;
                bestT = T;
